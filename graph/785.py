...
class UnionFind:
    ...
    def is_connected(self, node_1: int, node_2: int) -> bool:
        # print(self.find(node_1), self.find(node_2))
        return self.find(node_1) == self.find(node_2)


class Solution:

    def _dsu_approach(self, graph: list[list[int]]) -> bool:
        num_nodes = len(graph)
        uf = UnionFind(graph)
        ...
        for node in range(num_nodes):
            for next_node in graph[node]:
                if uf.is_connected(node, next_node):
                    return False
                print(graph[node][0], next_node)
                uf.union(graph[node][0], next_node)

        return True
        
    def _dfs_approach(self, graph: list[list[int]]) -> bool:
        num_nodes = len(graph)
    ...
    def _bfs_approach(self, graph: list[list[int]]) -> bool:
        num_nodes = len(graph)
        color = [0 for _ in range(num_nodes)]
    ...
    def is_bipartite(self, graph: list[list[int]]) -> bool:
        ...
        # return self._dfs_approach(graph_copy)
        # return self._bfs_approach(graph_copy)
        return self._dsu_approach(graph_copy)
