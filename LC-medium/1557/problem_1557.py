...
Output: [0,3]
Explanation: It's not possible to reach all the nodes from a single 
vertex. From 0 we can reach [0,1,2,5]. From 3 we can reach [3,4,2,5]. 
So we output [0,3].
...
other node, so we must include them. Also any of these vertices can 
reach nodes 1 and 4.
'''
...
class Solution:
    ...
    def findSmallestSetOfVertices(self,
        ...
        indegree = [False for _ in range(n)]

        for edge in edges:
            node_from, node_to = edge

            indegree[node_to] = True
